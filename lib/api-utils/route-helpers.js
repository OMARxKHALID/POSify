...
export const validateWithZod = (schema, data) => {
  // Generic schema type detection
  let schemaType = "ZodObject";
  let schemaKeys = "Unknown";

  if (schema._def?.shape) {
    schemaKeys = Object.keys(schema._def.shape);
    schemaType = `ZodObject(${schemaKeys.join(", ")})`;
  } else if (schema._def?.typeName) {
    schemaType = schema._def.typeName;
  } else if (schema._def?.left?._def?.shape) {
    // Handle extended schemas (e.g., baseSchema.extend())
    schemaKeys = Object.keys(schema._def.left._def.shape);
    schemaType = `ZodExtended(${schemaKeys.join(", ")})`;
  }

  console.log("🔍 [DEBUG] validateWithZod called:", {
    data,
    schemaType,
    schemaKeys,
    hasSchema: !!schema,
  ...
  console.log("🔍 [DEBUG] Zod validation result:", {
    ...
    data: result.data,
    error: result.success ? "No errors" : result.error?.issues,
  });
  ...
  console.log("✅ [DEBUG] Validation successful, returning data");
  return result.data;
};
  ...
  return async (request) => {
    try {
      ...
      if (requireValidation && schema) {
        ...
        console.log("✅ [DEBUG] Validation successful:", validatedData);
      } else {
        console.log("🔍 [DEBUG] Skipping validation:", {
          requireValidation,
          hasSchema: !!schema,
        });
        validatedData = body;
      ...
      if (error.message === "VALIDATION_ERROR") {
        console.log("❌ [DEBUG] Validation error occurred:", {
          zodError: error.zodError,
          issues: error.zodError?.issues,
          message: error.zodError?.message,
        });
