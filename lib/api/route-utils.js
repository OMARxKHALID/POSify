...
export const createRouteHandler = (handler, options = {}) => {
  ...
  return async (request, context = {}) => {
    try {
      ...
      // Call the actual handler with unified parameters
      let result;
      if (requireBody) {
        // POST/PUT/PATCH routes: pass (validatedData, request, params)
        result = await handler(validatedData, request, params);
      } else {
        // GET/DELETE routes: merge queryParams and route params for convenience
        const queryParams = parseQueryParams(request);
        const unifiedParams = { ...queryParams, ...params };
        result = await handler(unifiedParams, request);
      }
